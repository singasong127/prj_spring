package com.mycompany.app.infra.weather;

public class Area {
	
	private String seq;
	private String country;
	private String areacode;
	private String step1;
	private String step2;
	private String step3;
	private String gridX;
	private String gridY;
	private String longitudeHour;
	private String longitudeMin;
	private String longitudeSec;
	private String latitudeHour;
	private String latitudeMin;
	private String latitudeSec;
	private String longitudeMs;
	private String latitudeMs;
	
	private Weather weather; // 날씨 정보
	
	public Area(String areacode, String step1, String step2, String gridX, String gridY) {
		this.areacode = areacode;
		this.step1 = step1;
		this.step2 = step2;
		this.gridX = gridX;
		this.gridY = gridY;
	}
	
	public void updateAreaWeather(Weather weather) {
		
		this.weather = weather;
		
	}
	
	public String toString() {
		return step1 + " " + step2;
	}
	
	
	public String getSeq() {
		return seq;
	}
	public void setSeq(String seq) {
		this.seq = seq;
	}
	public String getCountry() {
		return country;
	}
	public void setCountry(String country) {
		this.country = country;
	}
	public String getAreacode() {
		return areacode;
	}
	public void setAreacode(String areacode) {
		this.areacode = areacode;
	}
	public String getStep1() {
		return step1;
	}
	public void setStep1(String step1) {
		this.step1 = step1;
	}
	public String getStep2() {
		return step2;
	}
	public void setStep2(String step2) {
		this.step2 = step2;
	}
	public String getStep3() {
		return step3;
	}
	public void setStep3(String step3) {
		this.step3 = step3;
	}
	public String getGridX() {
		return gridX;
	}
	public void setGridX(String gridX) {
		this.gridX = gridX;
	}
	public String getGridY() {
		return gridY;
	}
	public void setGridY(String gridY) {
		this.gridY = gridY;
	}
	public String getLongitudeHour() {
		return longitudeHour;
	}
	public void setLongitudeHour(String longitudeHour) {
		this.longitudeHour = longitudeHour;
	}
	public String getLongitudeMin() {
		return longitudeMin;
	}
	public void setLongitudeMin(String longitudeMin) {
		this.longitudeMin = longitudeMin;
	}
	public String getLongitudeSec() {
		return longitudeSec;
	}
	public void setLongitudeSec(String longitudeSec) {
		this.longitudeSec = longitudeSec;
	}
	public String getLatitudeHour() {
		return latitudeHour;
	}
	public void setLatitudeHour(String latitudeHour) {
		this.latitudeHour = latitudeHour;
	}
	public String getLatitudeMin() {
		return latitudeMin;
	}
	public void setLatitudeMin(String latitudeMin) {
		this.latitudeMin = latitudeMin;
	}
	public String getLatitudeSec() {
		return latitudeSec;
	}
	public void setLatitudeSec(String latitudeSec) {
		this.latitudeSec = latitudeSec;
	}
	public String getLongitudeMs() {
		return longitudeMs;
	}
	public void setLongitudeMs(String longitudeMs) {
		this.longitudeMs = longitudeMs;
	}
	public String getLatitudeMs() {
		return latitudeMs;
	}
	public void setLatitudeMs(String latitudeMs) {
		this.latitudeMs = latitudeMs;
	}
	
	
}
